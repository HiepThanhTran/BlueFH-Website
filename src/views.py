...
from src import app, db, login, services
...
def password_reset(token):
    return render_template(
        template_name_or_list="customer/password_reset.html", token=token
    )
...
def signin():
    if request.method.__eq__("POST"):
        ...
        account = services.authenticate(
            username=username_signin, password=password_signin
        )
...
def forgot_password():
    if request.method.__eq__("POST"):
        ...
        html = render_template(
            "mail/password_reset_email.html", recovery_url=recovery_url
        )
...
def reset_with_token(token):
    if request.method.__eq__("POST"):
        ...
        if user.email != email:
            ...
            services.update_account_password(
                account_id=user.account_id, new_password=new_password
            )
...
def appointment():
    if request.method.__eq__("POST"):
        ...
        services.create_examination_schedule(
...
def employee_nurse():
    if request.method.__eq__("POST"):
        ...
        services.create_examination_list(
            examination_date=day_of_exam,
            nurse_id=current_user.user.employee.nurse.id,
            examination_schedule_id_list=schedules,
        )

        flash(
            "Create an examination list and send notifications to patients successfully",
            "success",
        )
    ...
    return render_template(
        template_name_or_list="employee/nurse.html",
...
def employee_doctor():
    ...
    return render_template(
        template_name_or_list="employee/doctor.html",
...
def employee_cashier():
    ...
    return render_template(
        template_name_or_list="employee/cashier.html",
        bill_list=bill_list,
        medical_bills_list=medical_bills_list,
    )
